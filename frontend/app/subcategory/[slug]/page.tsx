...
import { productsAPI, subCategoriesAPI, brandsAPI } from '@/src/lib/api';
import { Product, SubCategory, Brand } from '@/src/types/index';
...
export default function SubCategoryPage() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [subCategory, selectedBrand, sortBy, sortOrder, currentPage, priceRange]);

  const fetchSubCategory = async () => {
    try {
      const all = await subCategoriesAPI.getAll();
      const found = all.find((sc) => sc.slug === slug);
      setSubCategory(found || null);
    } catch (err) {
      console.error('Error fetching subcategory:', err);
      setError('Could not load subcategory');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="relative h-48 md:h-64 bg-linear-to-r from-pink-100 to-purple-100 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <div className="text-center text-white">
              ...
              {subCategory.description && <p className="text-lg opacity-90">{subCategory.description}</p>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <nav className="flex items-center gap-2 text-sm text-gray-600 mb-6">
          ...
          {subCategory?.Type?.Category && (
            <>
              <ChevronRight className="w-4 h-4" />
              <Link href={`/category/${subCategory.Type.Category.slug}`} className="hover:text-pink-600">
          ...
          {subCategory?.Type && (
            <>
              <ChevronRight className="w-4 h-4" />
              <Link href={`/type/${subCategory.Type.slug}`} className="hover:text-pink-600">
        ...
        {!subCategory?.image && (
          <div className="mb-6">
            ...
            {subCategory?.description && <p className="text-gray-600">{subCategory.description}</p>}
          </div>
        )}
